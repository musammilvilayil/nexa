...
def test_chat_lists_tools(tmp_path: Path):
    agent = NexaAgent(build_default_registry(tmp_path / "workspace"), SessionManager(tmp_path / "sessions"))
    reply = agent.chat("show tools")
    assert reply.session_id
    assert "workspace.read" in reply.message


def test_chat_write_then_read(tmp_path: Path):
    agent = NexaAgent(build_default_registry(tmp_path / "workspace"), SessionManager(tmp_path / "sessions"))
